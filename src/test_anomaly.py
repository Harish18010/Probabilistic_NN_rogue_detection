...
def test_distance_anomaly():
    ...
    legit_x, _, _ = next(iter(legit_loader))
    with torch.no_grad():
       
        legit_database = extract_features(model, legit_x.to(device))
    ...
    legit_test_x, _, _ = next(iter(legit_loader))
    legit_test_x = legit_test_x[:5]
